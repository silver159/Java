...
//		System.out.println(vo.title); // private 권한으로 선언된 변수에 직접 접근하면 에러가 발생한다.
...
//		"=="을 사용해서 같은가 비교할 수 있는 데이터 기본 자료형 8가지와 null만 가능하다.
...
//		String은 단일 데이터가 저장되는 클래스라서 equals() 메소드만 사용하면 저장된 내용이 같으가 다른가
//		비교할 수 있었지만 사용자 정의 클래스 객체는 단일 데이터가 아니기 때문에 별도의 설정을 하지 않으면
//		equals() 메소드로 비교할 수 없다.
		
//		별도의 설정을 하지 않으면 아래의 결과는 "다르다"가 출력된다.
//		클래스 객체를 비교할 때 객체를 구성하는 모든 필드의 내용이 같은 비교할 수 있도록 equals()
