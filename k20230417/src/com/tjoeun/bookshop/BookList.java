...
//		size = 10;
//		bookList = new BookVO[size]; // 배열의 방을 만든다.
		this(10);
	}

//	generate Constructor using field (alt+shift+s)
...
//	main에서 만들어 사용한다. bookList.addBook(book1);
//	일단 이름을 써서 만든다. 빨간줄 -> 클릭
	public void addBook(BookVO book)
	{
//		System.out.println("book: "+ book); // 잘 들어왔나 확인용
		/*
...
//		try 블록의 문장들을 실행하다가 예외가 발생되면 더 이상 try 블록의 문장을 실행하지 않고
...
//			str.trim();
			...
			System.out.println("배열이 가득차서 "+book.getAuthor()+" 도서 정보를 저장할 수 없습니다.");
//			getMessage(): 예외 메시지를 얻어온다. 잘 안쓴다. printStackTrace()에 다 나온다.
//			System.out.println(e.getMessage());
			
//			printStackTrace(): 예외 메시지를 출력하고 예외가 발생되기 전에 실행된 문장을 실행순의
//			역순으로 추적한다.
//			e.printStackTrace();
...
//			예외를 처리하는 모든 클래스는 Exception 클래스를 상속받아 만들어졌기 때문에 catch
