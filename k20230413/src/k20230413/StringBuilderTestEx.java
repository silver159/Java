...
public class StringBuilderTestEx
{
...
//		멤버 변수 = 필드
//		문자열을 연결할때 String 클래스처럼 문자열을 새로 만들이 않고
//		char[]배열을 변경한다.
...
//		appand() 메소드로 StringBuilder 클래스 객체에 문자열을 연결한다.
		builder.append(css);
		System.out.println(builder);
		System.out.println(System.identityHashCode(builder));
...
//		StringBuffer 멀티 스레드 프로그램에서 동기화(순서를 정한다.)를 보장하므로 멀티 스레드
//		프로그램은 StringBuffer를 사용하고 싱글(단일) 스레드 프로그램은 Stringbuilder를 사용하는
//		것을 권장한다.
