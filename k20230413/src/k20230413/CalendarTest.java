...
//	static(정적) 메소드는 메소드가 작성된 클래스 객체를 생성하지 않고 클래스 이름에 "."을 찍어서
...
//	년, 월, 일을 인수로 넘겨받아 1년 1월 1일 부터 지난 날짜의 합계를 계산해 리턴하는 메소드
...
//		전년도 12월 31일까지 지난 날짜의 합계에 전달까지 지난 날짜의 합계에 일을 더해서 리턴시킨다.
		return sum += day;
	}
...
//	프로그램을 실행할 수 없다. 그래서 main() 메소드는 누군가 호출하기 전에 JVM이 메모리에 적재시킨다.
	
	public static void main(String[] args)
	{
//	static 메소드는 static 메소드에만 전근할 수 있다.
...
//			출력한 날짜(i)거 토요일이고 그 달의 마지막날짜가 아니면 줄을 바꾼다.
