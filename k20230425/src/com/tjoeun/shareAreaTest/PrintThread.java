...
public class PrintThread extends Thread
{
	...
	{
		/*
		// CalculatorThread의 실행이 끝날 때(shareArea.ready에 true가 저장될 때)까지
		// 대기시킨다.
		System.out.print("헉.. 헉.. 열심히 계산중...");
		while(!shareArea.ready) {
			// 스레드가 너무 빠르게 실행되면 정상적인 처리가 되지 않기 때문에 sleep()
			// 메소드로 주기적으로 프로그램을 멈춰야 한다.
				try
				{
					System.out.print(".");
					sleep(100);
		...
		*/
		...
		// wait() 메소드는 현재 스레드를 다른 스레드에서 깨울 때까지 일시 정지시킨다.
		// wait() 메소드로 일시 정지된 스레드는 다른 스레드에서 notify(), notifyAll() 
		// 메소드를 실행한다.
		// notify() 메소드는 wait() 메소드로 일시 정지된 스레드를 임의의 순서로 깨운다.
		// notifyAll() 메소드는 일시 정지된 모든 스레드를 일괄적으로 깨운다,
		// wait(), notify(), notifyAll() 메소드는 모두 동기화(Synchronized) 블록 내부에
		// 코딩해야 한다.
