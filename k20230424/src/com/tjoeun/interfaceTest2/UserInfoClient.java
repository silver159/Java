...
//			System.out.println((char)fis.read()); // try~catch 사용 IOException
			...
			*/
			...
			properties.load(fis); // try~catch 사용 IOException
			...
			// 사용할 기본값을 지정할 수 있다. => 오류 방지 차원
//			System.out.println(properties.getProperty("DBTYPE", "없음"));
			
			dbType = properties.getProperty("DBTYPE", "없음");
			System.out.println("DBTYPE: "+dbType);
		} catch (FileNotFoundException e) // fis = new FileInputStream(filepath); 오류 방지
		{
//			e.printStackTrace();
			System.out.println("디스크에 파일에 존재하지 않습니다.");
		} catch (IOException e) // fis.read(),properties.load(fis) 오류 방지
		{
//			e.printStackTrace();
