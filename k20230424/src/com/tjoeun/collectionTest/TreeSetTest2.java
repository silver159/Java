...
public class TreeSetTest2
{
	public static void main(String[] args)
	{
		...
		tset.add("장길산");
		System.out.println(tset.size() + ": "+ tset );
		tset.add("일지매"); // 저장되는 순서와 입력 순서가 다르다.
		System.out.println(tset.size() + ": "+ tset );
		tset.add("홍길동"); // 중복 데이터 저장 x
		System.out.println(tset.size() + ": "+ tset );
		System.out.println("===========================");
		
		ArrayList<String> list = new ArrayList<>();
		// 인덱스 사용 못하기 때문에 일반 for 사용할 수 없다.
		...
		{
			System.out.println("arrayList 사용: "+list.get(i));
		}
		System.out.println("===========================");
		...
		while(iterator.hasNext()) // Iterator 인터페이스 객체에 데이터가 있는 동안 반복한다.
		{
			...
			// next() 메소드를 Iterator 인터페이스 객체의 다음 데이터를 얻어온다.
