...
public class TreeSetTest3Ex
{
	public static void main(String[] args)
	{
		TreeSet<PersonEx> tset = new TreeSet<>();
		...
		// Comparable 인터페이스를 구현하고 compareTo() 메소드를 Override 해서 정렬 기준으로
		// 사용할 필드에 저장된 데이터를 비교하는 코드를 만들어야 한다.
		
		tset.add(new PersonEx("홍길동", 20));
		System.out.println(tset.size() + ": "+ tset );
		tset.add(new PersonEx("홍길동", 20));
		System.out.println(tset.size() + ": "+ tset );
		tset.add(new PersonEx("홍길동", 35));
		System.out.println(tset.size() + ": "+ tset );
		tset.add(new PersonEx("홍길동", 27));
		System.out.println(tset.size() + ": "+ tset );
		tset.add(new PersonEx("임꺽정", 27));
		System.out.println(tset.size() + ": "+ tset );
		tset.add(new PersonEx("장길산", 20));
		System.out.println(tset.size() + ": "+ tset );
		tset.add(new PersonEx("일지매", 12));
		System.out.println(tset.size() + ": "+ tset );
