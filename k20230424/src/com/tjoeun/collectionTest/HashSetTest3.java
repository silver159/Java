...
public class HashSetTest3
{
	public static void main(String[] args)
	{
		...
		// 같은(hashcode가 같은) 객체를 2번 이상 넣으면 종복되는 객체로 인식해서 저장하지 않는다.
		hset.add(person2);
		System.out.println(hset.size() + ": "+ hset );
		*/
		...
		// 저장된 내용이 같은 객체를 같은 객체로 인식하게 하려면 hashCode() 메소드를 
		// Override 해서 내용이 같은 객체는 같은 hashcode를 가지게 하면 된다. 
		...
		System.out.println(hset.size() + ": "+ hset );
		hset.add(new Person("임꺽정", 27));
		System.out.println(hset.size() + ": "+ hset );
		hset.add(new Person("장길산", 20));
		System.out.println(hset.size() + ": "+ hset );
