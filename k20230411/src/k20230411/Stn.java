...
//		최대값을 기억할 기억장소는 아주 작은 값을, 최소값을 기억할 기억장소는 아주 큰 값을 초기치로 지정한다.
		...
		for(int i = 0; i<data.length;i++)
		{
			...
			if (data[i]<min) //else if (x) => 오름차순일 경우 사용안될 수 있다.
			{
				min = data[i];
			}
		}
...
//		i가 1부터 시작하는 이유는 최대값과 최소값을 기억할 변수에 배열의 0번째 인덱스의 값으로 초기치로 지정했고
//		자기 자신끼리 비교해봐야 최대값과 최소값을 판별하는데 전현 도움이 되지 않기 때문이다.
...
//		최대값과 최소값을 제외한 나머지 데이터의 평균을 계산한다.
		int sum = 0;
//		0번째 인덱스에서 최소값이 n-1번째 인덱스에서 최대값이 저장되어 있으므로 최대값과 최소값이 저장된
//		인덱스는 제외하고 합계를 계산한다.
...
//			stn = temp*temp;
		...
		System.out.printf("분산: %f%n",var);
//		double std = Math.pow(var,0.5);
		double std = Math.sqrt(var);
		System.out.printf("표준편차: %f%n",std);
	}
}
