...
//		1항과 2항의 값은 위에서 이미 합계가 계산된 상태이므로 3번째 항 부터 n번째 항까지 합계를 누적한다.
		for (int k=3; k<=n; k++) {
			int c = a + b; // 피보나치 수역의 n-2 번째 항과 n-1 번째 항을 더해서 n번째 항 값을 계산한다.
			y += c; // 피보나치 수열의 합계를 계산한다.
//			피보나치 수열의 다음 항 값을 계산하기 위해 변수값을 수정한다.
			a = b; // 현재 작업의 n-1 번째 항의 값을 다음 작업의 n-2 번째 항으로 넣어준다.
			b = c; // 현재 작업의 n 번째 항의 값을 다음 작업의 n-1 번째 항으로 넣어준다.
		}
