...
//		trim(): 문자열 앞, 뒤의 붚리요한 빈칸(공백)을 제거한다. 자주 사용됨
		System.out.println(str.trim());
		System.out.println("불필요한 빈칸을 제거한 문자 개수: "+str.trim().length());
		
//		System.out.print("문자열 입력: ");
//		데이터 입력 단계에서 불필요한 공백을 제거하고 입력받으려면 아래와 같이
//		trim() 메소드를 사용하면 된다.
//		String name = scanner.nextLine().trim();
//		System.out.println(name);
		
		str = "8304223185600";
//		charAt(6): 문자열에서 지정된 index 번째 위치의 문자를 얻어온다.
//		index는 0부터 시작한다.
...
//		System.out.println(ch);
		...
		str = "tjoeunIT";
//		toUpperCase() 메소드는 영문자를 무조건 대문자로 변환하고
//		toLowerCase() 메소드는 영문자를 무조건 소문자로 변환한다.
		System.out.println(str.toUpperCase());
		System.out.println(str.toLowerCase());
		
		str = "8304223185600";
//		substring(a): 문자열의 a번째 index의 문자부터 문자열의 끝까지 얻어온다.
//		substring(a,b): 문자열의 a번째 index의 문자부터 'b-1'번째 index의 문자까지 얻어온다.
...
//		indexOf(), lastIndexOf() 메소드의 실행 결과가 0이상이면 인수로 지정된 문자열이
//		포함되었다는 의미로 사용할 수 있다.
...
//		}
		...
		System.out.printf("%s-%s-%s%n",newCode2[0],"*".repeat(newCode2[1].length()),newCode2[2]);
		
		newCode = String.format("%s-%s-%s%n",newCode2[0],"*".repeat(newCode2[1].length()),newCode2[2]);
		System.out.println(newCode);
	}
}
