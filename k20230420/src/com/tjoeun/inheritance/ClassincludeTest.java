...
//	자바는 일반적으로 파일 1개에 1개의 클래스를 만들어 사용하지만 필요에 따라서 
// 	1개의 파일에 여러개의 클래스를 만들어 사용할 수 있다.
//	1개의 파일에 여러개의 클래스를 만들면 "*.java" 파일은 1개가 만들어 지지만 
//	컴파일되면 "*.class" 파일은 각각 독립적으로 만들어진다.
//	현재 java 파일의 이름과 같은 이름의 클래스만 public을 붙일 수 있다.

//  기본 생성자가 실행되면 현재 날짜로 필드를 초기화하고 년, 월, 일을 넘겨받는
...
//기본 생성자가 실행되면 현재 시간으로 필드를 초기화하고 시, 분, 초를 넘겨받는
