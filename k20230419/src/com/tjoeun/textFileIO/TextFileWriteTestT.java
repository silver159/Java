...
public class TextFileWriteTestT {
...
//		절대경로는 작업중인 파일이 위치한 디스크 드라이브의 최상위(root) 폴더(디렉토디)부터
//		작업중인 파일이 위치한 폴더까지의 경로를 말한다.
		
//		상대경로는 작업중인 파일이 위치한 프로젝트 이름의 폴더(".")부터 작업중인 파일이 위치한
//		폴더까지의 경로를 말한다.
//		python이나 c/c++의 "."은 화면에 열려있는 파일이 위치한 폴더를 의미하지만 java의 "."은
//		파일이 포함된 프로젝트 이름의 폴더를 의미한다.
		
//		경로 지정시 경로와 경로, 경로와 파일을 구분하는 "\"가 기본이지만 에러가 발생되면 "\\"나
//		"/"로 수정한다.
