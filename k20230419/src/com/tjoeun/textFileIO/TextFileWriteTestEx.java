...
public class TextFileWriteTestEx
{
	public static void main(String[] args)
	{
		Scanner scanner = null; // 키보드 입력에 사용할 Scanner
		PrintWriter printWriter = null; // 텍스트 파일 출력에 사용할 PrintWriter
//		try catch 밖에서 선언, 안에서 생성

//		경로 지정 방식은 절대경로 지정방식과 상대경로 지정방식이 있다.
//		절대경로는 작업중인 파일이 위치한 디스크 드라이브의 취상위(root) 폴더(디렉토리)부터
...
//		String filepath = ".\\src\\com\\tjoeun\\textFileIO\\out.txt";
		...
		try
		{
//			PrintWriter 클래스의 생성자로 출력파일의 경로와 이름을 지정해서 출력파일을 만든다.
...
//				write() 메소드로 PrintWrite 클래스 객체로 생성한 파일에 출력한다.
...
//			출력파일은 출력 작업이 완료되면 반드시 파일을 close() 메소드로 닫아야 
//			정상적으로 파일이 생성된다.
			if(printWriter !=null) {
			printWriter.close();
			}
