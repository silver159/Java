...
public class StringTokenizerTestT {
...
//		split() 메소드는 구분자를 2개 이상 지정할 수 없으므로 공백과 탭이 섞여있는 데이터에
//		구분자를 공백만 지정하면 탭으로 구분된 데이터는 분리할 수 없다.
		str = "사과 배 복숭아	밤 대추";
...
//		생략시 false가 기본값으로 구분자를 token에 포함시키지 않지만 true를 쓰면 token을 구분자에 포함시킨다.
