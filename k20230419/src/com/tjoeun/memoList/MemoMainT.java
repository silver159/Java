...
//		MemoVO 클래스 객체에 저장한다.
		MemoVOT original = memoList.selectMemo(idx);
...
//	키보드로 삭제할 글번호를 입력받아 삭제할 글 1건을 얻어와 화면에 표시하고 비밀번호를 입력받아
...
//	키보드로 이름, 비밀번호, 메모를 입력받아 MemoVO 클래스 객체에 저장하고 ArrayList에 저장하는
//	메소드를 호출하는 메소드
...
//		MemoVO 클래스 객체에 입력받은 내용을 저장한다.
		MemoVOT vo = new MemoVOT(name, password, memo);
