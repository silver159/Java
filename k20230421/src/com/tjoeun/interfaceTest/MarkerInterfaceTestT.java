...
interface RepairableT { // SVC가 수리 가능한 유닛 클래스에 구현한다.
//	아무것도 코딩하지 않는다.
}
...
//	상수를 초기화시키는 방법은 3가지가 있는데 1번째 방법은 상수 선언시 "="을 사용해서
//	초기화시키는 방법이고 2번째 방법은 객체가 생성될 때 생성자에서 초기화 시킨다.
//	3번째 방법은 static으로 선언한 상수를 초기화하는데 2문장 이상 필요한 경우
//	static {}와 같이 초기화 블록을 사용하는 방법이 있다.
	final int MAX_HP; // 최대 HP, 상수
	int currentHP; // 현재 HP
	
//	생성자를 선언하지 않으면 자바 컴파일러가 아무런 일도 하지 않는 기본 생성자를 만들어
//	주지만 기본 생성자가 아닌 다른 생성자를 선언하면 컴퍼일러는 기본 생성자를 자동으로
//	만들어주지 않는다.
...
class GroundUnitT extends UnitT {

	public GroundUnitT(int hp) {
...
class AirUnitT extends Unit {

	public AirUnitT(int hp) {
		super(hp);
	}
...
class TankT extends GroundUnitT implements RepairableT {

	public TankT() {
//		② Tank 클래스의 기본 생성자가 실행되면 150을 인수로 부모 클래스(GroundUnit)의
//		생성자 GroundUnit(int hp)를 호출한다.
		super(150);
//		⑦ Tank의 MAX_HP가 150으로 초기화되었으므로 현재 HP(상속받는 currentHP)를 MAX_HP로
//		초기화시킨다.
		currentHP = MAX_HP;
...
class MarineT extends GroundUnitT {
...
class SCVT extends GroundUnitT implements RepairableT {
...
//	인터페이스를 구현받은 모든 클래스 또는 인터페이스 객체를 인수로 받을 수 있다.
	...
	public void repair(RepairableT repairable) {
		
//		표시 인터페이스 타입으로 수리할 객체를 받았는데 표시 인터페이스는 아무런 내용이 
//		코딩되어있지 않기 때문에 인수로 넘겨받은 객체를 실제로 구현한 내용이 들어있는
//		클래스로 형변환 시킨 후 처리해야 한다.
		
		if (repairable instanceof UnitT) {
			
			UnitT unit = (UnitT) repairable;
...
class DropShipT extends AirUnitT implements RepairableT {
...
public class MarkerInterfaceTestT {

	public static void main(String[] args) {
		
//		① Tank 클래스의 기본 생성자를 호출해서 Tank 클래스의 객체를 만든다.
		TankT tank = new TankT();
		MarineT marine = new MarineT();
		SCVT scv = new SCVT();
		DropShipT dropShip = new DropShipT();
