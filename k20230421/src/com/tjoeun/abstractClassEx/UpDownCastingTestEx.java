...
public class UpDownCastingTestEx
{
	public static void main(String[] args)
	{
		// 부모 클래스 타입으로 부모 클래스 객체를 만들어 사용하면 아무 문제 없이 처리된다.
		baseEx base = new baseEx();
		base.name = "홍길동";
		base.say();
		System.out.println("=======================");
		
		// 자식 클래스 타입으로 자식 클래스 객체를 만들어 사용하면 아무 문제 없이 처리된다.
		SubEx sub = new SubEx();
		sub.name = "임꺽정";
		sub.age = 20;
		sub.say();
		...
		// 부모 클래스 타입에 자식 클래스 타입의 객체가 생성된 주소를 대입하면 아무 문제없이 
		// 처리된다.
//		baseEx baseEx2 = new SubEx(); // 정상 실행
//		baseEx2.say();
		
		// 자식 클래스 타입에 부모 클래스 타입의 객체가 생성된 주소를 대입하면 에러가 발생한다.
//		SubEx ex2 = new baseEx(); // 에러 발생
		...
		// say() 메소드를 실행하면 부모 클래스의 say() 메소드가 아닌 자식 클래스의 say()
		// 메소드가 실행된다.
		b.say();
		System.out.println("=======================");
		
		// 자식 클래스 타입의 객체에 부모 클래스 타입의 객체가 생성된 주소를 대입한다.
//		SubEx s = baseEx; // 에러 발생
		
		// 자식 클래스 타입에 부모 클래스 타입의 객체나 객체가 생성된 주소를 대입하면
		// 에러가 발생되는데 이때, casting 시켜서 대입하면 대입이 가능하고 정상인 downcasting이
		// 아닌 경우 문법적으로 오류는 발생하지만 실행할 때 ClassCastException이 발생된다.
//		SubEx s = (SubEx)baseEx;
		
		// instanceof 연산자는 객체가 instanceof 뒤에 지정한 클래스 타입으로 안전하게 
		// 형변환이 가능한지 검사한다.
		
		if (sub instanceof baseEx)
		{
			System.out.println("sub 클래스 타입의 객체는 Base 클래스 타입으로 형변환 가능");
			baseEx base2 = (baseEx)sub;
			base2.say();
		}else
		{
		...
		// 정상적인 downcasting
		baseEx base2 = sub; // 부모 클래스 타입에 자식 클래스 타입을 대입한다. upcasting
		// downcasting은 부모 클래스 타입으로 upcasting된 것을 다시 자식 클래스 타입에 넣어주는
		// 것을 말한다.
		if (base2 instanceof SubEx)
		{
			SubEx sub2 = (SubEx) base2; // <= downcasting
			sub2.say();
		}else
		{
