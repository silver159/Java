...
// MemoService 클래스에서 전처리 작업이 완료되서 넘어온 데이터로 sql 명령을
// 실행하는 클래스
public class MemoDAO
{
	...
	public static ArrayList<MemoVO> select()
	{
		...
		try
		{
			...
			// ResultSet 객체에 데이터가 있는 동안 반복하며 ResultSet객체에 
			...
			while (rs.next())
			{
				...
				vo.setIdx(rs.getInt("idx")); // "idx" 필드이름
				vo.setName(rs.getString("name")); // "name" 필드이름
