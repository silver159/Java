...
// 전에 필요한 전처리 작업을 실행하는 클래스, 비지니스 로직을 작성하는 클래스
public class MemoServiceEx
{
	...
	public static void insert()
	{
		...
		Boolean result = MemoDAOEX.insert(vo);
	...
	// MemoDAOEX 클래스의 메소드를 호출하는 메소드
	public static void select()
	{
		// 테이블에 저장된 메모 목록을 얻어온다.
		ArrayList<MemoVO> list = MemoDAOEX.select();
	...
	public static void delete()
	{
		...
		MemoVO original = MemoDAOEX.selectByIdx(idx);
	...
	public static void update()
	{
		...
		if(original != null)
		{
			...
			if (original.getPassword().equals(password))
			{
				System.out.print("수정할 글의 메모를 입력하세요: ");
				String memo = scanner.nextLine().trim();
				
				// 비밀번호가 일치하면 MemoDAO 클래스의 메모를 수정하는 메소드를 호출한다. 
				MemoDAOEX.update(idx, memo);
